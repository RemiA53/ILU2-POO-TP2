...
public class BoundaryAcheterProduit {
	...
	public void acheterProduit(String nomAcheteur) {
		...
		if (!acheteurReconnu) {
			...
			String[] nomVendeurs = controlAcheterProduit.produitVenduMarche(produit);
			boolean isProduitVendu = nomVendeurs != null;
			procederAchat(nomAcheteur, isProduitVendu, produit, nomVendeurs);
			}
		}
	
	private void procederAchat(String nomAcheteur, boolean isProduitVendu, String produit, String[] nomVendeurs) {
		if (!isProduitVendu) {
			...
			chaine.append(" ?\n");
			for (int i = 1; i < nomVendeurs.length+1; i++) {
				chaine.append(i);
				chaine.append(" - ");
				chaine.append(nomVendeurs[i-1]);
			}
			int choix = Clavier.entrerEntier(chaine.toString());
			while(choix > nomVendeurs.length) {
				choix = Clavier.entrerEntier("Entrez une valeur valide !");
			}
			String nomVendeur = nomVendeurs[choix -1];
